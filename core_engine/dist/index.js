...
const AutonomousCoderAgent_1 = require("./agents/AutonomousCoderAgent");
const PersonalAssistantAgent_1 = require("./agents/PersonalAssistantAgent");
dotenv.config();
async function bootstrap() {
    ...
    const sandbox = new TerminalSandbox_1.TerminalSandbox(workspaceRoot);
    const fileManager = new SafeFileManager_1.SafeFileManager(workspaceRoot);
    const gateway = new GatewayServer_1.GatewayServer(port);
    ...
    // 2. Initialize Agents
    const coderAgent = new AutonomousCoderAgent_1.AutonomousCoderAgent(router, sandbox, fileManager, gateway);
    const assistantAgent = new PersonalAssistantAgent_1.PersonalAssistantAgent(gateway, router);
    ...
    gateway.on('chat_received', async (payload) => {
    ...
    gateway.on('command_execute', async (payload, clientWs) => {
        ...
        try {
            const result = await sandbox.executeCommand(command, { cwd: workspaceRoot }, async (cmd, reason) => {
    ...
    gateway.on('read_file', async (payload, clientWs) => {
        try {
    ...
    gateway.on('save_file', async (payload, clientWs) => {
        try {
